...
public class Call extends RobotAction {
		...
		if(ClientControl.session_mode != null && ClientControl.session_mode.equals("WEBRTC")) {
			...
			if(AudiOn) {
				...
				if(ApprtcService.status != ConnectionStatus.Connected){
					xmpp.sendMsg("R2C", String.format(result,"fail",2,1), 92902);
					/*获取audio资源*/
					...
					xmpp.sendMsg("R2C", String.format(result,"success",2,1), 92902);
				rtc.stopVideo();
				rtc.startAudio();
			} else {
				rtc.stopAudio();
			...
			if (AudiOn) {
				if((ZhuMuService.status == MeetingStatus.MEETING_STATUS_IDLE) && (ZhuMuService.isMeetingSDKInitOK == true)) {
					...
					//RobotAudioRecorder.getInstance().releaseRecord();
					ModuleController.getInstance().changeState(State.CLOSE, null);
					String musicpath = "sound/remote_audio.mp3";//path
					palyrecord(musicpath);
					xmpp.sendMsg("R2C", String.format(result,"success", 2,1), 92902);
					zhumu.joinRoom(ClientControl.ROOMID, XmppService.auth_Key,ZHUMUMeetingType.AUDIO);
				} else {
					RobotDebug.d(NAME, "ClientControl.ROOMID : " + ClientControl.ROOMID + "  XmppService.auth_Key " + XmppService.auth_Key + "机器人处于会议中，拒绝会议");
					xmpp.sendMsg("R2C", String.format(result,"fail", 2,2), 92902);
					zhumu.leaveRoom();
				}
			} else {
